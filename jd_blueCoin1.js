...
let coinToBeans = [0,0,0]; //兑换多少数量的京豆（20或者1000），0表示不兑换，默认兑换20京豆，如需兑换把0改成20或者1000，或者'商品名称'(商品名称放到单引号内)即可
let num;
    ...
    if (cookie) {
      ...
      if ($.isNode()) {
        if (process.env.MARKET_COIN_TO_BEANS) {
          coinToBeans[num] = process.env.MARKET_COIN_TO_BEANS;
        }
      }
      for (var j = 0; j < 10; j++) {
        try {
          if (`${coinToBeans[num]}` !== '0') {
            await smtgHome();//查询蓝币数量，是否满足兑换的条件
            await PrizeIndex();
          } else {
            console.log('查询到您设置的是不兑换京豆选项，现在为您跳过兑换京豆。如需兑换，请去BoxJs设置或者修改脚本coinToBeans[num]或设置环境变量MARKET_COIN_TO_BEANS\n')
          }
          await msgShow();
        } catch (e) {
          $.logErr(e)
  ...
  if (prizeList && prizeList.length) {
    if (`${coinToBeans[num]}` === '1000') {
      ...
      for (let index = 0; index < prizeList.length; index ++) {
        if (prizeList[index].title.indexOf(coinToBeans[num]) > -1) {
      ...
      if (prizeId) {
        if (prizeList[i].inStock === 506 || prizeList[i].inStock === -1) {
          console.log(`失败，您输入设置的${coinToBeans[num]}领光了，请明天再来`);
          $.beanerr = `失败，您输入设置的${coinToBeans[num]}领光了，请明天再来`;
          return ;
        }
        ...
        console.log(`奖品兑换列表[${coinToBeans[num]}]已下架，请检查APP是否存在此商品，如存在请检查您的输入是否正确`);
        $.beanerr = `奖品兑换列表[${coinToBeans[num]}]已下架`;
      }
    }
  }
}
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans[num]}` === '1000') {
                ...
                $.beanscount ++;
                console.log(`【京东账号${$.index}】${$.nickName || $.UserName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`)
                if ($.data.data.result.exchangeNum === 20 || $.beanscount === coinToBeans[num] || $.data.data.result.blue < 500) return;
              } else {
                $.beanscount ++;
                console.log(`【京东账号${$.index}】${$.nickName || $.UserName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`)
                if ($.beanscount === 1) return;
...
//通知
function msgShow() {
  // $.msg($.name, ``, `【京东账号${$.index}】${$.nickName}\n【收取蓝币】${$.coincount ? `${$.coincount}个` : $.coinerr }${coinToBeans[num] ? `\n【兑换京豆】${ $.beanscount ? `${$.beanscount}个` : $.beanerr}` : ""}`);
  return new Promise(async resolve => {
    $.log(`\n【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans[num] ? `【兑换${$.title}】${$.beanscount ? `成功` : $.beanerr}` : "您设置的是不兑换奖品"}\n`);
    ...
    if ($.beanscount && $.ctrTemp) {
      $.msg($.name, ``, `【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans[num] ? `【兑换${$.title}】${ $.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}`);
      allMessage += `【京东账号${$.index}】${$.nickName || $.UserName}\n${coinToBeans[num] ? `【兑换${$.title}】${$.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr}` : "您设置的是不兑换奖品"}${$.index !== cookiesArr.length ? '\n\n' : ''}`
      // if ($.isNode()) {
